# Found PowerShell -> chase execution chain - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **found powershell** to continue from a confirmed lead into its surrounding activity. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
| ImageFileName=/(\\powershell(_ise)?\.exe$|\\pwsh\.exe$)/i OR CommandLine=/(\\powershell(_ise)?\.exe$|\\pwsh\.exe$)/i
