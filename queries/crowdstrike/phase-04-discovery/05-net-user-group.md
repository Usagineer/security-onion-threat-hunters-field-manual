# Discovery — net user / net group - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **net user group** to identify commands used to learn the host, users, domain, and network. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
| ImageFileName=/(\\net1?\.exe$.*\b(user|group|localgroup)\b|Get-(LocalUser|LocalGroup|ADUser|ADGroup))/i OR CommandLine=/(\\net1?\.exe$.*\b(user|group|localgroup)\b|Get-(LocalUser|LocalGroup|ADUser|ADGroup))/i
