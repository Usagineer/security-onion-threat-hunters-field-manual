# Discovery — tasklist - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **tasklist** to identify commands used to learn the host, users, domain, and network. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
| ImageFileName=/(\\tasklist\.exe$|\btasklist\b|Get-Process|\bps\s+(aux|ef))/i OR CommandLine=/(\\tasklist\.exe$|\btasklist\b|Get-Process|\bps\s+(aux|ef))/i
