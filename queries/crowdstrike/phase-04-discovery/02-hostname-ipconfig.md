# Discovery — hostname / ipconfig - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **hostname ipconfig** to identify commands used to learn the host, users, domain, and network. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
| ImageFileName=/(\\hostname\.exe$|\\ipconfig\.exe$|\bhostname\b|\bipconfig\b|ifconfig|ip\s+addr)/i OR CommandLine=/(\\hostname\.exe$|\\ipconfig\.exe$|\bhostname\b|\bipconfig\b|ifconfig|ip\s+addr)/i
