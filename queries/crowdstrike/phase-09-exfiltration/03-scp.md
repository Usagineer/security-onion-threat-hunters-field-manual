# Exfiltration — SCP - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **scp** to identify staging and transfer of collected data. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
| ImageFileName=/(\\scp\.exe$|\bscp\s+|pscp)/i OR CommandLine=/(\\scp\.exe$|\bscp\s+|pscp)/i
