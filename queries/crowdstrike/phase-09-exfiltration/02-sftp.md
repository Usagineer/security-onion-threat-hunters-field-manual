# Exfiltration — SFTP - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **sftp** to identify staging and transfer of collected data. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
| ImageFileName=/(\\sftp\.exe$|\bsftp\s+|psftp)/i OR CommandLine=/(\\sftp\.exe$|\bsftp\s+|psftp)/i
