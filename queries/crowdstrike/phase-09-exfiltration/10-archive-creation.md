# Exfiltration — Archive Creation - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **archive creation** to identify staging and transfer of collected data. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
| ImageFileName=/(7z(\.exe)?.*\s+a\s+|rar(\.exe)?.*\s+a\s+|Compress-Archive|tar\s+(-c|.*cz)|makecab)/i OR CommandLine=/(7z(\.exe)?.*\s+a\s+|rar(\.exe)?.*\s+a\s+|Compress-Archive|tar\s+(-c|.*cz)|makecab)/i
