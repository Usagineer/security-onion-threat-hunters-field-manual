# Exfiltration — FTP - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **ftp** to identify staging and transfer of collected data. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
| ImageFileName=/(\\ftp\.exe$|\bftp\s+|lftp|curl.*ftp:)/i OR CommandLine=/(\\ftp\.exe$|\bftp\s+|lftp|curl.*ftp:)/i
