# Lateral Movement — PsExec - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **psexec** to identify movement from one system or account to another. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
| ImageFileName=/(psexec|psexesvc|\\ADMIN\$|accepteula.*\\\\)/i OR CommandLine=/(psexec|psexesvc|\\ADMIN\$|accepteula.*\\\\)/i
