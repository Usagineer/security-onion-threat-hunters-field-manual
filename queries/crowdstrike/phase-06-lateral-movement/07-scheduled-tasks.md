# Lateral Movement — Scheduled Tasks - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **scheduled tasks** to identify movement from one system or account to another. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
| ImageFileName=/(schtasks.*\/(create|run)|Register-ScheduledTask|New-ScheduledTaskAction)/i OR CommandLine=/(schtasks.*\/(create|run)|Register-ScheduledTask|New-ScheduledTaskAction)/i
