# Lateral Movement — WMI - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **wmi** to identify movement from one system or account to another. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
| ImageFileName=/(wmic.*\/node:.*process.*call.*create|Invoke-WmiMethod|Invoke-CimMethod|WmiCreateProcess)/i OR CommandLine=/(wmic.*\/node:.*process.*call.*create|Invoke-WmiMethod|Invoke-CimMethod|WmiCreateProcess)/i
