# ICS/OT — CIP - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **cip** to identify unsafe or unauthorized industrial-control activity. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
```cql
network.protocol=/^cip$/i OR cip.service=*
...
```cql
network.protocol=/^cip$/i OR cip.service=*
| groupBy([source.ip, destination.ip, cip.service], function=[count(as=events), sum(network.bytes, as=bytes)], limit=max)
