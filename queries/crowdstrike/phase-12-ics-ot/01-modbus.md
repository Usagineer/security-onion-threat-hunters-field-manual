# ICS/OT — Modbus - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **modbus** to identify unsafe or unauthorized industrial-control activity. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
```cql
network.protocol=/^modbus$/i OR destination.port=502
...
```cql
network.protocol=/^modbus$/i OR destination.port=502
| groupBy([source.ip, destination.ip, modbus.function], function=[count(as=events), sum(network.bytes, as=bytes)], limit=max)
