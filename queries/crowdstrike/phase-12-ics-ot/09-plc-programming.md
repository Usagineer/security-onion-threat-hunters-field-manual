# ICS/OT — PLC Programming / Logic Download - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **plc programming** to identify unsafe or unauthorized industrial-control activity. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
```cql
(event.action=/program|download|write|start|stop|reset|force/i OR ot.operation=/program|download|write|start|stop|reset|force/i)
| groupBy([source.ip, destination.ip, event.action], function=[count(as=events), sum(network.bytes, as=bytes)], limit=max)
