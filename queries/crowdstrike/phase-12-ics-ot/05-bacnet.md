# ICS/OT — BACnet - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **bacnet** to identify unsafe or unauthorized industrial-control activity. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
```cql
network.protocol=/^bacnet$/i OR destination.port=47808
...
```cql
network.protocol=/^bacnet$/i OR destination.port=47808
| groupBy([source.ip, destination.ip, bacnet.service], function=[count(as=events), sum(network.bytes, as=bytes)], limit=max)
