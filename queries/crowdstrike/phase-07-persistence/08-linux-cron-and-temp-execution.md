# Persistence — Linux Cron and Temporary-Path Execution - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **linux cron and temp execution** to identify mechanisms that preserve attacker access. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
| ImageFileName=/((crontab|cron).*(\/tmp|\/var\/tmp|\/dev\/shm)|\/(tmp|var\/tmp|dev\/shm)\/\S+)/i OR CommandLine=/((crontab|cron).*(\/tmp|\/var\/tmp|\/dev\/shm)|\/(tmp|var\/tmp|dev\/shm)\/\S+)/i
