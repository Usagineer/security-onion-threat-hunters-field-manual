# TLS for this IP - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **tls** to scope an IP lead across independent telemetry. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
```cql
(network.protocol=/^tls$/i OR tls.server.name=*) | source.ip=?{ip=*} OR destination.ip=?{ip=*}
