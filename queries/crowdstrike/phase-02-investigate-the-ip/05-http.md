# HTTP for this IP - CrowdStrike Next-Gen SIEM

## What this does

Hunts for **http** to scope an IP lead across independent telemetry. The result keeps the endpoint, account, process, network peer, and time fields needed to validate the behavior and continue scoping.
...
```cql
(network.protocol=/^http$/i OR url.full=*) | source.ip=?{ip=*} OR destination.ip=?{ip=*}
